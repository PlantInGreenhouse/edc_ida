...
class EDC:
            ...
            if model_type == "hf":
                ...
                if model_name == "text-embedding-ada-002":
                    ...
                    try:
                        ...
                        from edc.utils.llm_utils import OpenAIEmbeddingWrapper
                    model = OpenAIEmbeddingWrapper(model_name)
                else:
    ...
    def schema_canonicalization(
        ...
        if free_model:
            ...
            if sc_verify_model is not None and sc_verify_tokenizer is not None:
                llm_utils.free_model(sc_verify_model, sc_verify_tokenizer)
            self.loaded_model_dict.pop(self.sc_llm_name, None)
            self.loaded_model_dict.pop(self.sc_embedder_name, None)

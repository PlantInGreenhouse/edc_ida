...
def parse_raw_entities(raw_entities: str):
    parsed_entities = []
    if "[" not in raw_entities or "]" not in raw_entities:
        logging.warning(f"Malformed entity string (no brackets): {raw_entities}")
    ...
    try:
        ...
        logging.warning(f"Failed to parse entities: {e}")
...
def get_embedding_openai(text: str, model: str = "text-embedding-ada-002"):
    openai.api_key = os.getenv("OPENAI_KEY")
